...
class BloodFlowSolverJacobi(BloodFlowSolver):
    """
    Subclass of BloodFlowSolver to allow overriding the number of Jacobi iterations
...
def run_hybrid_simulation(pinn_model, solver, device, severity, heart_rate, T_cycle, steps_per_cycle, dt, fluid_indices, k_interval, beta, jacobi_iters):
    ...
    for step in range(1, steps_per_cycle + 1):
        if step % k_interval == 0:
            ...
            # Project pressure with custom iters
...
def main():
    ...
    iters_list = [0, 2, 5, 10, 20, 40, 80]
    beta = 0.95
    
    print(f"Sweeping Jacobi Iterations for Hybrid 4:1 (beta={beta}):")
    print(f"{"Iters":5} | {"FPS":6} | {"Time/Frame (ms)":15} | {"Velocity Error":14}")
    print("-"*48)
    
    for iters in iters_list:
        elapsed, states = run_hybrid_simulation(pinn_model, solver, device, severity, heart_rate, T_cycle, steps_per_cycle, dt, fluid_indices, 5, beta, iters)
        fps = steps_per_cycle / elapsed
        ms_per_frame = (elapsed / steps_per_cycle) * 1000
        err = calc_error(states)
        print(f"{iters:5d} | {fps:6.1f} | {ms_per_frame:15.2f} | {err:13.2%}")
